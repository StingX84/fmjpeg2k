    ...
    const DJPEG2KCodecParameter *djcp) const
{
    ...
    if (result.good())
    {
        ...
        l_stream = opj_stream_create_memory_stream(&mysrc, OPJ_UINT32(size), OPJ_FALSE);
        ...
        if (result.good())
        {
            ...
            compressedSize = (unsigned long)(size);
    ...
    const FMJPEG2KRepresentationParameter *djrp) const
{
    ...
    result = frametoimage(buffer, 0, photometricInterpretation, samplesPerPixel, width, height, bitsAllocated, pixelRepresentation, &parameters, &image);
    ...
    else
    {
        ...
        if(djrp->getnearlosslessPSNR() == 0)
            parameters.tcp_distoratio[0] = (bitsAllocated == 8)? 50.0f : 80.0f; // 50 or 80db depending on 8 or 16
        else
    ...
    l_stream = opj_stream_create_memory_stream(&mysrc, OPJ_UINT32(compressed_buffer_size), OPJ_FALSE);
    ...
    if (result.good())
    {
        ...
        compressedSize = (unsigned long)(compressed_buffer_size);
...
OFCondition frametoimage(const Uint8 *framePointer, int planarConfiguration, OFString photometricInterpretation, int samplesPerPixel, int width, int height, int bitsAllocated, OFBool isSigned, opj_cparameters_t *parameters, opj_image_t **ret_image)
{
    ...
    if(bytesAllocated == 1)
    {
        if(planarConfiguration == 0)
        {
            for(int i = 0; i < samplesPerPixel; i++)
            {
                ...
                for (unsigned int pos = 0; pos < pixelsCounts; pos++)
                {
        ...
        else
        {
            ...
            {
                OPJ_INT32 *target = image->comps[i].data;
                const Uint8 *source = &framePointer[i * pixelsCounts * bytesAllocated];
                for (unsigned int pos = 0; pos < pixelsCounts; pos++)
                {
                    *target =  (isSigned) ? (Sint8) *source : *source;
                    target++;
    ...
    else if(bytesAllocated == 2)
    {
        ...
        else
        {
            ...
            {
                OPJ_INT32 *target = image->comps[i].data;
                const Uint16 *source = (Uint16 *) &framePointer[i * pixelsCounts * bytesAllocated];
                for (unsigned int pos = 0; pos < pixelsCounts; pos++)
                {
                    *target =  (isSigned) ? (Sint16) *source : *source;
                    target++;
    ...
    else if(bytesAllocated == 4)
    {
        ...
        else
        {
            ...
            {
                OPJ_INT32 *target = image->comps[i].data;
                const Uint32 *source = (Uint32 *) &framePointer[i * pixelsCounts * bytesAllocated];
                for (unsigned int pos = 0; pos < pixelsCounts; pos++)
                {
                    *target =  (isSigned) ? (Sint32) *source : *source;
                    target++;
